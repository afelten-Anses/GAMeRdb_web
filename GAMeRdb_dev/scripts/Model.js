...
function findAttribute(attribute,value) // Parse GAMeRdb data filtered by attribute=value and return only the macthing attribute
{
...
function filterByAttribute(attribute,value) // Parse GAMeRdb data filtered by attribute=value and return all the matching JSON
{
	//console.log(`${attribute} for  ${value}`) //dev trace

	db.GENOME.find({"Phylogeny.Serovar":"Typhimurium"},function (err, docs) 
	{
    	var MongoJSON = docs;
    	MongoJSON = JSON.stringify(MongoJSON);
    	console.log(MongoJSON) // dev trance
    	return MongoJSON
	})
}
